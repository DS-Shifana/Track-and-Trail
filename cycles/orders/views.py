...
from .forms import OrderForm, ShippingAddressForm, UserForm,CouponForm 
...
def payment(request, quantity=0):
    ...
    if orders:
            ...
            if orders.applied_coupon:
                total -= orders.applied_coupon.discount_amount
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            if coupon.valid_upto < timezone.now():
                ...
                return redirect('payment')  # Redirect to an error page or handle accordingly
            elif order.total_amount < coupon.min_purchase_amount:
                messages.error(request, f'Amount should be greater than {coupon.min_purchase_amount}')
                return redirect('your_error_view')  # Redirect to an error page or handle accordingly
